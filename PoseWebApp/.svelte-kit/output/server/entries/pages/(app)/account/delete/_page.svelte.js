...
function _page($$renderer, $$props) {
  $$renderer.component(($$renderer2) => {
    ...
    $$renderer2.push(`<!----></a> <div><h1 class="text-2xl font-bold text-red-500">${escape_html(store_get($$store_subs ??= {}, "$_", $format)("account.danger_zone"))}</h1> <p class="text-sm text-slate-500 dark:text-slate-400">${escape_html(store_get($$store_subs ??= {}, "$_", $format)("account.delete_account"))}</p></div></div> `);
    {
      $$renderer2.push("<!--[-->");
