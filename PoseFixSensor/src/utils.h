...
typedef struct {
  uint8_t id;  // 1 byte para identificar el sensor (0 = cabeza, 1 = espalda)
  uint16_t pitch;
  // 2 bytes para pitch normalizado y mapeado (-15.4->345.6) -> 34560
  uint16_t roll;
  // 2 bytes para roll normalizado y mapeado (-15.4->345.6) -> 34560
  uint16_t yaw;
  // 2 bytes para yaw normalizado y mapeado (-15.4->345.6) -> 34560
} SensorData;

typedef struct {
  struct {
    uint8_t status : 1;
    // bit 0 para estado (0 = ok, 1 = fallo)
    uint8_t calibration : 1;
    // bit 1 para calibración (0 = ok, 1 = no calibrado)
    uint8_t power_source : 1;
    // bit 2 para estado de carga (0 = bateria, 1 = usb)
    uint8_t battery_level : 5;
    // bit 3-7 para carga de bateria (0-100%)
  };
  uint8_t byte;
} PayloadStatus;
...
typedef struct {
  ...
  uint8_t tag[8];        // 7 bytes para la etiqueta
} Payload;
#pragma pack(pop)
